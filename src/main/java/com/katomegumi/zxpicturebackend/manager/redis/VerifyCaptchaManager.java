package com.katomegumi.zxpicturebackend.manager.redis;
...
public class VerifyCaptchaManager {
    ...
    public void putCaptchaIntoRedis(String captcha,String to){
        String key= RedisConstant.EMAIL.REGISTER+to;
        BoundHashOperations<String, String, String> boundHashOps = stringRedisTemplate.boundHashOps(key);
        //获取最后一次发送邮件的事件
        String lastSendTime = boundHashOps.get(RedisConstant.EMAIL.LAST_SEND_TIME);
        //获取一共发送多少邮件
        String sendCount = boundHashOps.get(RedisConstant.EMAIL.COUNT);

        if (StringUtils.isBlank(lastSendTime) && StringUtils.isBlank(sendCount)) {
            boundHashOps.put(RedisConstant.EMAIL.CAPTCHA, captcha);
            boundHashOps.put(RedisConstant.EMAIL.LAST_SEND_TIME, String.valueOf(System.currentTimeMillis()));
            boundHashOps.put(RedisConstant.EMAIL.COUNT, "1");
        ...
        boundHashOps.put(RedisConstant.EMAIL.CAPTCHA, captcha);
        boundHashOps.put(RedisConstant.EMAIL.LAST_SEND_TIME, String.valueOf(System.currentTimeMillis()));
        boundHashOps.put(RedisConstant.EMAIL.COUNT, String.valueOf(sendCount!=null?Integer.parseInt(sendCount)+1:"1"));
        //验证码5分钟过期
        boundHashOps.expire(5,TimeUnit.MINUTES);
    }
    ...
    public boolean verifyCaptchaOk(String key,String captcha) {
        return ObjectUtil.equals(stringRedisTemplate.opsForHash().get(key,RedisConstant.EMAIL.CAPTCHA),captcha);
    }
